...
function movieRowHtml(movie) {
  ...
  const rating = movie.imdb ? `<a class="imdb-badge" href="${escapeHtml(movie.imdbLink || movie.link || "#")}" target="_blank" rel="noreferrer">${escapeHtml(movie.imdb)}</a>` : "";
  return `
    <article class="panel movie-row">
      ...
      <div>
        ...
        <div class="movie-links"><a href="${escapeHtml(movie.link || "#")}" target="_blank" rel="noreferrer">${escapeHtml(movie.linkLabel || "More info")} →</a></div>
      </div>
